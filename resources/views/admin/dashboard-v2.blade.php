    ...
    <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div class="flex flex-col gap-3 sm:flex-row">
            ...
            <form action="{{ route('produccion.cerrar') }}" method="POST">
                @csrf
                <button type="submit" onclick="return confirm('Â¿Cerrar la quincena actual y generar informe imprimible?')" class="rounded-full bg-rose-600 px-5 py-3 text-sm font-semibold text-white hover:bg-rose-700">
                    Cerrar quincena
                </button>
            </form>
        </div>
                ...
                <form action="{{ route('admin.usuarios.store') }}" method="POST" class="mt-6 space-y-4">
                    ...
                    <div class="rounded-3xl border border-slate-200 p-4">
                        ...
                        <label class="mt-3 flex items-center gap-3 text-sm text-slate-700">
                            ...
                            <span>Permitir edicion de precios si el rol es recolector</span>
                        </label>
                    </div>
                        ...
                        <div class="rounded-[1.5rem] border border-slate-200 p-4">
                            <form action="{{ route('admin.usuarios.update', $usuario) }}" method="POST">
                                ...
                                <div class="grid gap-3 md:grid-cols-2">
                                    ...
                                    <div class="rounded-2xl border border-slate-200 p-4 text-sm text-slate-700 md:col-span-2">
                                        ...
                                        <label class="flex items-center gap-3">
                                            ...
                                            <span>Permitir edicion de precios cuando el rol sea recolector</span>
                                        </label>
                                    </div>
                                ...
                                <div class="mt-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                                    <div class="flex flex-wrap items-center gap-2 text-sm text-slate-500">
                                        <span>{{ $usuario->name }} | {{ $usuario->cedula ?: 'Sin cedula' }} | {{ $usuario->contacto ?: 'Sin contacto' }}</span>
                            ...
                            <div class="mt-3 flex flex-wrap gap-2">
                                <form action="{{ route('admin.usuarios.toggle-status', $usuario) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <button onclick="return confirm('Â¿Cambiar el estado de este usuario?')" class="rounded-full border px-4 py-2 text-sm font-semibold {{ $usuario->activo ? 'border-amber-200 text-amber-700 hover:bg-amber-50' : 'border-emerald-200 text-emerald-700 hover:bg-emerald-50' }}">
                                ...
                                <form action="{{ route('admin.usuarios.destroy', $usuario) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button onclick="return confirm('Â¿Eliminar este usuario?')" class="rounded-full border border-rose-200 px-4 py-2 text-sm font-semibold text-rose-700 hover:bg-rose-50">
                                        Eliminar
                                    </button>
                                </form>
                            </div>
