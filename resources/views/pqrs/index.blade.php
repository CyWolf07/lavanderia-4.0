...
<div class="space-y-6">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Acciones</th>
                        </tr>
                    </thead>
    ...
    <!-- BOTÓN EDITAR -->
    <a href="{{ route('pqrs.edit', $pqrs->id) }}"
       class="px-3 py-1 bg-yellow-400 hover:bg-yellow-500 text-white rounded-lg text-xs font-semibold shadow">
        Editar
    </a>

    <!-- BOTÓN ELIMINAR -->
    <form action="{{ route('pqrs.destroy', $pqrs->id) }}" method="POST">
        @csrf
        @method('DELETE')

        <button type="submit"
                onclick="return confirm('¿Seguro que deseas eliminar este registro?')"
                class="px-3 py-1 bg-red-500 hover:bg-red-600 text-white rounded-lg text-xs font-semibold shadow">
            Eliminar
        </button>
    </form>
